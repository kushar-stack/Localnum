...
import { clusterArticles, summarizeArticle } from "./logic.js";
import {
  renderHero,
  renderActiveFilters,
  renderMetrics,
...
export async function fetchNews({ reset = false, force = false } = {}) {
  ...
  try {
    ...
    if (reset) {
      ...
      if (elements.metricsGrid) elements.metricsGrid.innerHTML = "";
      if (elements.news) {
...
export function downloadBrief() {
  ...
  appState.currentBrief.forEach((article, i) => {
    ...
    const primarySource = article.source?.name || article.sourceMeta?.[0]?.name || "Global News";
    if (!article.summary?.bullets?.length || !article.summary?.why) {
      article.summary = summary;
    }
    content += `${i + 1}. ${article.title.toUpperCase()}\n`;
    content += `Source: ${primarySource}\n`;
    if (article.summary?.bullets) {
      article.summary.bullets.forEach(b => content += `  - ${b}\n`);
    }
    content += `Why it matters: ${article.summary?.why || summary.why || "Key development."}\n\n`;
  });
